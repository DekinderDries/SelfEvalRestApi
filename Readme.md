...
The front-end will be a beautiful designed javascript-based application that will communicate with an even more beautiful engineered back-end application.
...
* o Instead, use **eurder**.
* Timing
* To properly implement all the stories, in a test-first way with a proper design and architecture, 
* an experienced developer will need around 2-3 days, it might be more, it might be less. 
* Therefore, we've created two categories of stories: Must-Have's and Nice-To-Have's. Start with the Must-Have's, then do the Nice-To-Have's.
...
* Perform logging 
o Certainly log all interactions with the application that can be defined as "errors" 
...
o Setup a proper test configuration, which runs the integration tests against an in-memory database (e.g. H2) 
* Make it a separate technical story.
o Correctly setup and handle the transactions.
o Write your DDL (create tables,...) in a separate .sql file, which you also put under version control.
* Use Jenkins to set up a Continuous Integration (CI) pipeline. 
o Additionally, but optional, deploy to Heroku!
...
o Until then, you can neglect the fact that certain endpoints should only be usable by - for example - an administrator.
...
o Certainly log all interactions with the application that can be defined as "errors" - E.g.: unauthorized access, illegal arguments, exceptions in general,...
...
* o The shipping date should be calculated automatically: 
...
* o Tip: The the price of the item can change over time... What implications might this have?
...
o The id(entifier) of the order
o Per item group of the order 
* The name of the item
* The ordered amount
* The total price of the item group
o The total price of the order
...
o E.g.: Give me the list of items with urgency indicator STOCK_LOW
...
A new feature request was made by the business of Örder. For every Item, they would like to be able to set a state (pristine, slightly damaged or damaged).
...
o You will now have to provide a different modeling. Each "Fridge Coolio Z" Item will represent a single fridge.
